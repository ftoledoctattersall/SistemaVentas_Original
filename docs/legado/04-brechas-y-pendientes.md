...
| PEN-004 | Funcionalidad aparentemente obsoleta | La pantalla de calzada propia existe, pero no se encontró en sus scripts ni en servidor un flujo alcanzable que registre borrador u orden. Su vigencia no está confirmada. | MEDIA | Confirmar menú productivo y uso con usuarios; ejecutar un caso si la opción está habilitada. |
...
| PEN-006 | Regla de negocio no determinable | El código confirma que las excepciones llevan a borrador SAP y que, tras aprobación, el usuario lo procesa desde el monitor; siguen pendientes los umbrales exactos que deciden orden directa versus autorización. | ALTA | Revisar SP, datos productivos y pruebas por modalidad. |
...
| PEN-018 | Dependencia del desarrollador original | Se confirmó que el ASMX `wssap` sólo publica cuatro operaciones, mientras `ventas` consume además un componente SAP para conversión, facturación y compras. Falta confirmar cuál combinación está desplegada y soportada en producción. | MEDIA | Revisar despliegue IIS, versiones de ensamblados y telemetría/logs productivos. |
...
| PEN-025 | Regla de negocio no determinable | La revalidación de crédito previa a facturación está implementada, pero no se puede determinar con seguridad la combinación exacta de saldo, días extra y estados que permite omitir o exigir la autorización adicional. | ALTA | Validar con Crédito y conciliar casos reales antes de diseñar el flujo futuro. |
